// animate sensitivity / specificity numbers + ring on load
  ...
  (function(){
    ...
    function bestGradEye(){
      ...
      return entries.reduce((a,b) => b[1].classification.grade > a[1].classification.grade ? b : a)[0];
    }
    ...
    function refreshReportEyeButtons(){
      ['L','R'].forEach(eye => {
        ...
        if (!btn) return;
        const hasData = !!state[eye];
        btn.classList.toggle('has-data', hasData);
        btn.classList.toggle('active', selectedReportEye === eye);
        btn.disabled = !hasData;
        btn.title = hasData ? `View ${eye === 'L' ? 'left' : 'right'} eye` : `Upload and run ${eye === 'L' ? 'left' : 'right'} eye first`;
      });
    }

    function setReportEye(eye){
      if (!state[eye] || !rawImages[eye]) return;
    ...
    function renderDetailedReport(eye, report){
      ...
      if (gradeTag){
        gradeTag.textContent = referable ? 'Referable — refer to ophthalmologist' : 'Not referable at this grade';
        gradeTag.className = `tag ${referable ? 'referable' : 'optimal'}`;
      }
      ...
      if (confidenceBar) confidenceBar.style.width = `${Math.max(0, Math.min(100, confidence * 100))}%`;
    ...
    function renderLesionViews(entries){
      ...
      const totals = lesionTypes.map(([, , key]) => entries.reduce((sum, [, data]) => sum + (Number(data.lesions?.[key]) || 0), 0));
      const averages = totals.map((total) => total / entries.length);
      const averageMax = Math.max(1, ...averages);
      if (chart){
        chart.innerHTML = lesionTypes.map(([short], index) => `<div class="lesion-chart-row"><span>${short}</span><div class="lesion-chart-track"><b style="width:${(averages[index] / averageMax) * 100}%"></b></div><strong>${averages[index].toFixed(1)}</strong></div>`).join('');
      }
    ...
    function updateCombined(){
      ...
      const worst = entries.reduce((a,b) => b[1].classification.grade > a[1].classification.grade ? b : a);
      ...
      document.getElementById('tryConfidence').textContent = `${(classification.confidence * 100).toFixed(1)}%`;
      const tag = document.getElementById('tryTag');
      if (classification.referable){
      ...
      // ---- drive the hero patient panel from the same worst-eye result ----
      document.getElementById('heroAwaiting').classList.add('hide');
      document.getElementById('heroGradeNum').textContent = classification.grade;
      document.getElementById('heroConfNum').textContent = `${(classification.confidence * 100).toFixed(1)}%`;
      const heroGradeTag = document.getElementById('heroGradeTag');
      const heroConfTag = document.getElementById('heroConfTag');
      heroGradeTag.style.visibility = 'visible';
      heroConfTag.style.visibility = 'visible';
      if (classification.referable){
        heroGradeTag.textContent = 'Referable — refer to ophthalmologist';
        heroGradeTag.className = 'tag referable';
      } else {
        heroGradeTag.textContent = 'Not referable at this grade';
        heroGradeTag.className = 'tag optimal';
      }
      heroConfTag.textContent = `Scan complete · ${entries.length === 2 ? 'both eyes' : eyeLabel}`;

      // ---- Lesion Detection card ----
      const totalMA = entries.reduce((sum, [,r]) => sum + (r.lesions?.microaneurysms || 0), 0);
      document.getElementById('lesionSub').textContent =
        `${totalMA} microaneurysms, exudates & haemorrhages localised`;
      document.getElementById('lesionSub').style.color = '#5C4620';

      // ---- Image Quality card ----
      const anyBorderline = entries.some(([,r]) => r.quality_gate?.recapture_required || r.quality?.overall === 'Fair');
      document.getElementById('qualitySub').textContent = anyBorderline
        ? 'Borderline on one field — recapture recommended for full confidence'
        : 'Adequate — focus, illumination and field of view all passed';
      ...
      document.querySelector('.report-doc-summary .report-conf-row b').textContent = `${(classification.confidence * 100).toFixed(1)}%`;
      document.querySelector('.report-doc-summary .tag').textContent = classification.referable ? 'Referable — refer to ophthalmologist' : 'Not referable at this grade';
      document.querySelector('.report-doc-summary .tag').className = `tag ${classification.referable ? 'referable' : 'optimal'}`;
    ...
    function renderLiveReportPreview(entries, worstEye, report){
      ...
      const eyeReports = entries.map(([eye, data]) => {
        ...
        return `<article class="preview-eye-report"><div class="preview-eye-heading"><h3>${eyeName}</h3><span class="tag ${referable ? 'referable' : 'optimal'}">${referable ? 'Referable' : 'Not referable'}</span></div><div class="preview-result-grid"><div><span class="preview-label">ICDR grade</span><strong>${classification.grade ?? '—'}</strong></div><div><span class="preview-label">Confidence</span><strong>${classification.confidence == null ? '—' : `${(classification.confidence * 100).toFixed(1)}%`}</strong><span class="preview-muted">Uncalibrated model output</span></div><div><span class="preview-label">Quality</span><strong>${quality.overall || '—'}</strong><span class="preview-muted">${gate.action || 'Quality gate complete'}</span></div></div><div class="preview-image-grid">${imageCard(data.submitted_photo_url, 'Input fundus image', `${eyeName} input fundus image`)}${imageCard(data.xai?.heatmap_url || data.result_image_url, 'Grad-CAM attention', `${eyeName} Grad-CAM attention`)}${imageCard(data.lesions?.annotated_url, 'Lesion detection', `${eyeName} lesion detection image`)}</div><div class="preview-lesions"><h3>Lesions detected</h3>${bars}</div><div class="preview-recommendation"><strong>Clinician recommendation</strong><span>${referable ? 'Further ophthalmological evaluation is recommended.' : 'Routine monitoring and follow-up according to clinical protocol.'}</span></div></article>`;
      }).join('');
      ...
      return {
        ...
        eyes: entries.map(([eye, data]) => {
          ...
          return {
            ...
            screening_category: classification.grade === undefined ? null : (referable ? 'Referable' : 'Non-Referable'),
            ai_recommendation: referable ? 'Further ophthalmological evaluation is recommended.' : 'Routine monitoring and follow-up according to clinical protocol.',
