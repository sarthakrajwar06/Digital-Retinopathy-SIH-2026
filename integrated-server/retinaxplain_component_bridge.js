// RetinaXplain bridge for Streamlit Components v2.
// The original dashboard remains unchanged. This bridge intercepts only
// /api/analyze and /api/report and sends those requests to Python.

export default function(component) {
  const { parentElement, data, setTriggerValue } = component;
  const root = parentElement;
  const document = parentElement.ownerDocument || window.document;
  ...
  const nativeFetch = window.__retinaxplain_native_fetch || window.fetch.bind(window);
  window.__retinaxplain_native_fetch = nativeFetch;
  ...
  // Install the transport shim before the original dashboard script runs.
  ...
  // Execute the original dashboard JS only once for this component instance.
  // This prevents duplicate event handlers after Streamlit reruns.
  if (!RX.initializedRoots.has(root)) {
    RX.initializedRoots.add(root);

    /* ORIGINAL_SCRIPT */
  }

  return () => {
    // Streamlit removes the component DOM itself. Pending requests are kept in
    // the global bridge object so a rerun can resolve them.
  };
}
